...
def load_timestamp_table(path: Path, direction: str, timestamp_col: str) -> pd.DataFrame:
    if not path.exists():
    ...
    out["timestamp"] = pd.to_numeric(df[timestamp_col], errors="coerce")
...
def plot_hourly(hourly: pd.DataFrame, out_dir: Path) -> None:
    ...
    for message_type, sub in hourly.groupby("message_type", sort=False):
        ax.plot(sub["hour"], sub["relative_frequency"], marker="o", label=message_type)
    ax.set_title("Message distribution by hour of day")
...
def plot_weekday(weekday: pd.DataFrame, out_dir: Path) -> None:
    ...
    width = 0.38
    x = np.arange(7)
    directions = list(weekday["direction"].drop_duplicates())
    for i, direction in enumerate(directions):
        sub = weekday[weekday["direction"] == direction].sort_values("weekday")
        offset = (i - (len(directions) - 1) / 2) * width
        ax.bar(x + offset, sub["relative_frequency"], width=width, label=MESSAGE_LABELS.get(direction, direction))
    ax.set_title("Message distribution by weekday")
...
def plot_weekday_hour_heatmap(weekday_hour: pd.DataFrame, out_dir: Path) -> None:
    directions = [d for d in ["direct", "group"] if d in set(weekday_hour["direction"])]
    if not directions:
        return

    matrices = {d: pivot_matrix(weekday_hour, d) for d in directions}
    ...
    for idx, direction in enumerate(directions):
        ...
        im = ax.imshow(matrix, aspect="auto", cmap="Greys", vmin=0, vmax=vmax, interpolation="nearest")
        panel = chr(ord("A") + idx)
        ax.set_title(f"{panel}. {MESSAGE_LABELS.get(direction, direction)}", loc="left", fontsize=11)
    ...
    if im is not None:
        cbar = fig.colorbar(im, ax=axes, orientation="vertical", fraction=0.028, pad=0.025)
        cbar.set_label("Messages within corpus (%)", fontsize=9)
        cbar.ax.tick_params(labelsize=8)
    fig.tight_layout(rect=[0, 0, 0.94, 1])
    save_figure(fig, out_dir, "combined_direct_group_weekday_hour_heatmap")
