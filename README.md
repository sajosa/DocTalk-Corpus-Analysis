# DocTalk Corpus Analysis

[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.22682263.svg)](https://doi.org/10.5281/zenodo.22682262)

Reproducible analysis pipeline for a German-language corpus of interprofessional clinical chat communication collected in the DocTalk project.

The repository contains source code, corpus-specific cleaning rules, aggregated public results, publication figures, and a fully synthetic demonstration dataset. The confidential source corpus is not included.

## Project overview

The analysis examines authentic interprofessional chat communication from a German university hospital. The main objectives are to:

- characterize temporal and interactional communication patterns;
- identify recurrent lexical and thematic structures;
- compare direct and group communication;
- examine communication related to clinical coordination, handover, scheduling, and organizational processes;
- derive implications for secure clinical messaging infrastructures and professional training.

The workflow was developed as part of a biomedical informatics and data science project and subsequently revised for publication-oriented corpus analysis.

## Purpose and intended audience

Clinical messaging corpora are difficult to share openly because even anonymized message-level communication may retain sensitive contextual information. This limits independent inspection, reproduction, and reuse of analytical workflows.

This repository addresses that problem by separating confidential source data and message-level outputs from a transparent public analysis pipeline. It provides documented cleaning decisions, aggregated empirical outputs, publication figures, and synthetic data that mirror the structure of selected intermediate corpus tables.

The repository is intended for researchers in health informatics, clinical communication, corpus linguistics, and reproducible computational research who wish to inspect or adapt methods for analysing secure clinical messaging.

It is a research reproducibility resource, not a clinical application, a production messaging system, or a source of reusable empirical clinical data.

## Project background and funding

This repository contains the analysis pipeline developed for a corpus study conducted in the context of the research project **“DocTalk – Dialog trifft Chatbot: Kollaborierendes Lernen und Lehren für Ärzt:innen im Prozess der Arbeit.”**

Further information about the project and its institutional context is available from:

- [Digital Medicine at Charité – Universitätsmedizin Berlin](https://psychosomatik.charite.de/forschung/digitale_medizin/)
- [DocTalk project at Charité – Universitätsmedizin Berlin](https://psychosomatik.charite.de/forschung/digitale_medizin/projekt_doctalk/)
- [DocTalk project website](https://doctalk.chat/)

The DocTalk project was funded by the German Federal Ministry of Education and Research (Bundesministerium für Bildung und Forschung, BMBF) under [grant number 01PG20002](https://qd.bmbfcluster.de/de/projektdatenbank-27.php?D=378&F=0&M=445&T=1), as part of the programme *Digital Media in Vocational Education* and its *DigiMed* funding line.

## Repository structure

```text
.
├── data/
│   └── synthetic_sample/         # Fully synthetic demonstration data
├── outputs/
│   └── public/                   # Aggregated public tables and figures
├── rules/                        # Cleaning rules and documented decisions
├── scripts/                      # Numbered analysis pipeline
├── src/                          # Shared cleaning functions
├── CITATION.cff                  # Citation metadata
├── requirements.txt              # Core dependencies
├── requirements_full.txt         # Extended development environment
├── LICENSE
└── README.md
```

Confidential corpus data and message-level review outputs are stored locally in:

```text
outputs/confidential/
outputs/results/
```

These directories are excluded from version control.

## Data availability

The original DocTalk corpus contains confidential clinical workplace communication and cannot be made publicly available.

This repository therefore includes a small, fully synthetic demonstration corpus in:

`data/synthetic_sample/`

The synthetic messages were created from templates and are not derived from original corpus messages. They do not represent real patients, employees, clinical cases, conversations, or events.

The synthetic sample mirrors the intermediate corpus tables produced by `01_build_corpus.py`. It can therefore be used to inspect the expected data structure and test the public workflow from lexical cleaning onward.

The synthetic sample does not reproduce the frequencies, effect sizes, collocations, temporal distributions, or other empirical findings of the original study.

Further information is available in [`data/synthetic_sample/README.md`](data/synthetic_sample/README.md).

## Installation

### macOS and Linux

```bash
python -m venv .venv
source .venv/bin/activate
```

### Windows (PowerShell)

```powershell
python -m venv .venv
.venv\Scripts\activate
```

Install the core dependencies:

```bash
python -m pip install --upgrade pip
pip install -r requirements.txt
```

For the extended development environment:

```bash
pip install -r requirements_full.txt
```

## Synthetic demonstration data

Regenerate the synthetic dataset with:

```bash
python scripts/00_generate_synthetic_sample.py
```

The generator creates:

```text
data/synthetic_sample/
├── D_utterances_raw.csv
├── D_metadata_raw.csv
├── G_utterances_raw.csv
└── G_metadata_validated.csv
```

These files match the schemas of the confidential intermediate tables generated by `01_build_corpus.py`.

Because the original raw export structure cannot be shared, `01_build_corpus.py` is not fully reproducible with the public sample. The public demonstration workflow therefore begins with lexical cleaning.

## Analysis pipeline

| Script | Purpose |
|---|---|
| `00_generate_synthetic_sample.py` | Generate the public synthetic demonstration corpus |
| `01_build_corpus.py` | Build direct and group corpus tables from confidential source exports |
| `02_clean_lexical.py` | Apply the first lexical cleaning and normalization stage |
| `03_validate_cleaning_markers.py` | Validate expected markers after cleaning |
| `04_frequency_ngram_keyness.py` | Generate frequency, n-gram, and direct-versus-group keyness results |
| `05_export_keyness_review_tables.py` | Export condensed keyness tables for manual review |
| `06_validate_unlinked_kein.py` | Inspect occurrences of the German negation marker `kein` |
| `07_create_clean_lexical_v2.py` | Create final lexical analysis tables and normalized multiword expressions |
| `08_run_targeted_collocations_kwic_v2.py` | Run targeted collocation and keyword-in-context analyses |
| `09_sort_collocation_review_v2.py` | Prepare collocation results for structured manual review |
| `10_validate_therapy_group_compounds.py` | Validate therapy-group multiword-expression normalization |
| `11_validate_kein_todo.py` | Validate normalization of `kein Todo` and related constructions |
| `12_medical_terminology_analysis.py` | Identify and review clinical terminology |
| `12b_export_validated_medical_terms_summary.py` | Export the manually validated terminology summary |
| `13_time_analysis.py` | Generate aggregated temporal distributions and temporal figures |
| `14_emoji_analysis.py` | Analyse emoji use in direct and group messages |
| `15_generate_figures.py` | Generate the final thread-length distribution figure |
| `15b_generate_medical_terminology_figure.py` | Generate the clinical terminology figure |
| `16_generate_accessible_lexical_figures.py` | Generate the direct-versus-group keyness figure and low-minus-high volume supplement |
| `17_generate_marker_weekday_matrix_by_modality.py` | Generate the weekday workflow-marker matrix by communication modality |

### Validation script ordering

`10_validate_therapy_group_compounds.py` documents the validation basis for the therapy-group multiword-expression normalization applied in `07_create_clean_lexical_v2.py`.

The numbering reflects the practical analysis workflow rather than a strict dependency order.

## Running the public demonstration workflow

From the project root, regenerate the synthetic corpus:

```bash
python scripts/00_generate_synthetic_sample.py
```

Run the first lexical cleaning stage:

```bash
python scripts/02_clean_lexical.py --corpus both --synthetic
```

This reads:

```text
data/synthetic_sample/D_utterances_raw.csv
data/synthetic_sample/G_utterances_raw.csv
```

and writes cleaned synthetic tables to:

`data/synthetic_sample/cleaned/`

Create the final v2 lexical tables:

```bash
python scripts/07_create_clean_lexical_v2.py --synthetic
```

This generates:

```text
data/synthetic_sample/cleaned/
├── D_utterances_clean_lexical_v2.csv
├── G_utterances_clean_lexical_v2.csv
├── utterances_for_collocation_clean_lexical_v2.csv
└── marker_presence_check_clean_lexical_v2.xlsx
```

The `--synthetic` option keeps demonstration runs separate from the confidential corpus and prevents synthetic test runs from overwriting empirical publication outputs.

Because the synthetic corpus is intentionally small, analyses that depend on minimum-frequency thresholds, stable effect-size estimation, collocation strength, or larger temporal distributions should be interpreted as software demonstrations only.

## Lexical cleaning approach

The final lexical analysis pipeline does not apply general stopword removal.

Instead, corpus-specific normalization rules reduce artificial token fragmentation while retaining function words, interaction markers, and negations that may carry communicative meaning in short clinical chat messages.

The cleaning process includes:

- normalization of anonymization placeholders;
- standardization of patient and colleague references;
- normalization of gender-inclusive word forms;
- preservation of analytically relevant lexical markers;
- normalization of selected multiword expressions;
- explicit handling of `Todo`, `kein Todo`, and `kein aktives Todo`;
- preservation of underscores in normalized multiword expressions.

Detailed cleaning and validation decisions are documented in:

[`rules/cleaning_decisions.md`](rules/cleaning_decisions.md)

Protected terms are listed in:

[`rules/protected_tokens.txt`](rules/protected_tokens.txt)

## Rationale for the analytical approach

The final workflow prioritizes transparent and auditable corpus-linguistic methods that correspond closely to the study’s descriptive and comparative research questions.

This decision reflects several characteristics of the corpus: individual messages are typically short, linguistically elliptical, highly context-dependent, and frequently contain anonymization placeholders or recurrent workflow expressions.

### Exploratory LDA topic modelling

Exploratory Latent Dirichlet Allocation (LDA) was tested during method development but was not retained.

Because many individual messages contained only a small number of lexical items, they provided limited within-document word co-occurrence information from which latent topics could be inferred reliably. This is a recognized challenge in conventional topic modelling of short texts and has motivated the development of specialized short-text models such as the Biterm Topic Model ([Yan et al., 2013](https://doi.org/10.1145/2488388.2488514)).

Aggregating individual messages into artificial longer documents might have reduced lexical sparsity. However, it would also have introduced consequential decisions about document boundaries and obscured message-level, temporal, and direct-versus-group distinctions that were central to the study.

### Exploratory BERT-based topic modelling

Transformer-based language models are not inherently unsuitable for short messages.

Exploratory BERT-based topic modelling was therefore conducted using BERTopic and multilingual transformer-derived sentence embeddings. BERTopic combines document embeddings, clustering, and class-based TF-IDF to construct topic representations ([Grootendorst, 2022](https://doi.org/10.48550/arXiv.2203.05794)).

In this corpus, however, manual review showed that the resulting clusters were frequently shaped by anonymization placeholders, formulaic expressions, and highly local workflow vocabulary. The identified topic structures did not provide sufficient coherence and clinical interpretability to support robust publication-level conclusions.

No externally validated topic labels were available against which the unsupervised solutions could be evaluated. The exploratory topic-modelling results were therefore not retained as final study findings.

### Final analytical strategy

The final publication pipeline comprises:

- frequency distributions and n-grams;
- direct-versus-group keyness;
- targeted collocation analysis;
- keyword-in-context review;
- validated medical terminology analysis;
- emoji analysis;
- temporal distributions;
- predefined workflow-marker analyses;
- contextual manual validation.

This approach favors interpretability, traceability, and reproducibility over broader but less readily verifiable latent semantic representations.

## Temporal analysis

Temporal analysis is implemented in:

[`scripts/13_time_analysis.py`](scripts/13_time_analysis.py)

It uses the timestamp columns of the cleaned direct- and group-message tables. Timestamps are expected as Unix timestamps in seconds.

Weekday and hour-of-day variables are derived from reconstructed message timestamps during analysis rather than from a pre-existing weekday field in the final lexical tables.

A complete run, including CSV exports, can be started with:

```bash
python scripts/13_time_analysis.py --write-csv
```

Aggregated public tables are written to:

`outputs/public/tables/time/`

The generated files include:

- `time_analysis_tables.xlsx`
- `time_analysis_summary.csv`
- `combined_messages_hourly_distribution.csv`
- `combined_messages_weekday_distribution.csv`
- `combined_messages_weekday_hour_distribution.csv`
- `weekday_hour_distribution_by_modality.csv`

The combined weekday-by-hour heatmap generated by `13_time_analysis.py` is used as the final publication figure for temporal message distributions. Additional hourly and weekday plots are retained as descriptive supplementary outputs.

## Workflow marker analysis

Selected workflow-related lexical markers are visualized with:

[`scripts/17_generate_marker_weekday_matrix_by_modality.py`](scripts/17_generate_marker_weekday_matrix_by_modality.py)

The analysis includes the normalized markers `Übergabe`, `WE`, `kein_Todo`, `Rückmeldung`, and `anwesend`.

For each communication modality and weekday, the script calculates the percentage of messages containing the respective marker at least once. Direct and group messages use separate modality- and weekday-specific denominators.

Weekdays are derived from reconstructed message timestamps. Weekend estimates should be interpreted cautiously because of substantially smaller message denominators.

Aggregated source tables are written to:

`outputs/public/tables/figure_sources/`

## Public outputs

Aggregated results suitable for public release are stored in:

`outputs/public/`

The principal publication figures are:

- thread-length distribution by communication modality;
- normalized frequencies of top direct- and group-associated keyness tokens;
- weekday-by-hour message distribution for direct and group communication;
- weekday distribution of selected workflow markers by communication modality.

Supplementary figures include the low-minus-high communication-volume lexical contrast and the validated clinical terminology figure.

Public outputs include:

- publication figures in PNG, PDF, and SVG formats;
- aggregated frequency and n-gram tables;
- keyness result tables;
- emoji summaries;
- temporal analysis tables;
- validated summaries of clinical terminology;
- figure source tables.

The public outputs do not contain original messages, keyword-in-context extracts, conversation-level text, speaker identities, or confidential review annotations.

## Confidential outputs

The following directories are excluded from Git:

```text
outputs/confidential/
outputs/results/
```

They may contain cleaned message-level corpus tables, keyword-in-context concordances, manual review files, candidate terminology tables, message-context validation files, intermediate analysis outputs, and confidential metadata.

These files must not be committed to a public repository.

## Reproducibility

### Computational reproducibility

The source code, cleaning rules, synthetic input schemas, synthetic demonstration data, and public aggregated outputs allow users to inspect and test the computational workflow.

### Empirical reproducibility

The exact empirical results cannot be independently regenerated without access to the confidential DocTalk corpus.

Access to the original corpus is restricted because the messages originate from a clinical workplace setting and may contain sensitive contextual information even after anonymization.

The synthetic sample is intended for software testing and structural inspection only.

## Design trade-offs and future development

The repository reflects several deliberate trade-offs:

- **Confidentiality over full empirical reproducibility:** The original corpus and message-level outputs cannot be shared. Public aggregated outputs and synthetic data provide computational transparency without exposing sensitive communication.
- **Context preservation over generic text preprocessing:** General stopword removal was avoided because function words, interaction markers, and negations may carry important meaning in short clinical messages.
- **Interpretability over model complexity:** Exploratory LDA and BERT-based topic modelling did not yield sufficiently coherent and clinically interpretable topic structures. The final workflow therefore prioritizes transparent corpus-linguistic measures and contextual validation.
- **Domain specificity over immediate portability:** Cleaning rules and normalized expressions were developed for this German-language clinical corpus and may require adaptation before use with other datasets.
- **Transparent research scripts over a packaged application:** The numbered scripts document the analytical workflow but are not intended to function as a general-purpose corpus-analysis tool.

Potential future improvements include:

- automated end-to-end workflow tests;
- continuous integration;
- more configurable input schemas, paths, and analysis parameters;
- stronger environment and dependency versioning;
- additional synthetic test cases;
- further documentation for adapting the workflow to other datasets;
- validation with other clinical messaging corpora where appropriate data governance permits.

These items describe possible future development rather than committed functionality.

## Ethics and confidentiality

The source communication data were collected in the context of the DocTalk project and processed under the applicable institutional, ethical, and data-protection requirements.

The public repository does not contain original clinical chat messages, personal identifiers, pseudonymized speaker mappings, patient-level records, message-level concordances, confidential metadata, or files enabling reconstruction of individual communication sequences.

All public tables were reviewed for confidentiality before release.

## Use of generative AI

Generative AI tools were used during repository development for selected tasks such as code review, debugging support, documentation drafting, and language editing.

All generated suggestions were manually reviewed, tested, and adapted by the author. Analytical decisions, corpus interpretation, validation, and responsibility for the final code and results remained with the author.

No confidential source corpus was intentionally submitted to a public generative AI service.

## Software quality checks

### Check Python syntax

```bash
python -m compileall scripts src
```

Test the public synthetic workflow with:

```bash
python scripts/00_generate_synthetic_sample.py
python scripts/02_clean_lexical.py --corpus both --synthetic
python scripts/07_create_clean_lexical_v2.py --synthetic
```

Check that confidential directories are not tracked:

```bash
git ls-files data/direct
git ls-files data/group
git ls-files outputs/confidential
git ls-files outputs/results
```

All four commands should return no files.

## Citation

For the archived software release, please cite the version-specific Zenodo record:

> Sayegh-Jodehl, S. (2026). *DocTalk Corpus Analysis* (Version 1.0.0) [Computer software]. Zenodo. https://doi.org/10.5281/zenodo.22682263

## License

The source code is distributed under the license specified in the `LICENSE` file.

The license applies to the source code and public documentation only. It does not grant access to, or usage rights for, the confidential DocTalk corpus.

## Contact

Questions regarding the analysis workflow, code, or publication outputs can be submitted through the GitHub issue tracker.